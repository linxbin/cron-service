CREATE TABLE `task_log` (
  ...
  `status` tinyint NOT NULL DEFAULT '0' COMMENT '0:失败，1:运行众，2:完成',
...
CREATE TABLE `task` (
  ...
  `status` tinyint(4) NOT NULL DEFAULT '0' COMMENT '1正常0停止',
  `created` datetime NOT NULL COMMENT '添加时间',
  `updated` datetime NOT NULL COMMENT '更新时间',
  `is_del` tinyint(4) NOT NULL DEFAULT '0' COMMENT '是否被删除',
  PRIMARY KEY (`id`)
...
CREATE TABLE `auth` (
  ...
  `is_del` tinyint(4) NOT NULL DEFAULT '0' COMMENT '是否被删除',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='认证管理表';
